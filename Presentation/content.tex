...
\begin{frame}
    {Актуальность темы}
    \justifying
    В настоящее время тенденция бурного развития процесса \textbf{«цифровизации»} нефтегазовой отрасли.
...
\begin{frame}
    \begin{center}
        {Положение 4 - математические модели в виде задачи ЦЛП и экстремальной комбинаторной задачи для оптимального размещения базовых станций при проектировании БШС с линейной топологией}
    \end{center}
\end{frame}
...
\begin{frame}
    ...
    Величина суммарного потока с объектов через систему станций должна поступить на шлюз.
    ...
    Задача (20) - (23) является задачей линейного программирования (ЛП) с функцией невязок уравнений ограничений в качество целевой функции.
...
\begin{frame}
    ...
    Задача чЦЛП также относятся к широкому к \textbf{классу задач размещения мощностей}. Отличительной особенностью рассмотренных задач является наличие условия на связь между размещаемыми объектами и линейная контролируемая территория.
    \bigskip
...
\begin{frame}
    ...
    По среднии значениям и стандартным отклонениям проводилась апроксимация функций распределений. Если коэффициент вариации $c_v = \frac{\sigma}{m} < 1$, то распределение принадлежит распределения Эрланга. Если $c_v =1$, то экспоненциальному закону. Если $c_v > 1$, то случайная величина принадлежит гиперэкспоненциальное распределению. 
...
\begin{frame}
    ...
    \hfill
    \begin{minipage}[c]{0.5\linewidth}
        \includegraphics[width=0.92\textwidth]{total_scatter_diagram.png}
